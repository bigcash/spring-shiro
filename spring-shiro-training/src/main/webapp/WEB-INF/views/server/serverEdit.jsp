	...
	$(function() {
		...
		$.post("${path }/user/queryUsers", function(data) {
			...
			for (var i = 0; i < bToObj.length; i++) {
				$("#marcher").append("<option value=" + bToObj[i].key + ">" + bToObj[i].value + "</option>");
			}
			$("#marcher").select2('val', $("#marcher").val());
		});
...
<div class="easyui-layout" data-options="fit:true,border:false">
	<div data-options="region:'center',border:false" title="" style="overflow: hidden; padding: 3px;">
		<table class="grid">
			...
			<tr>
				<td>序号</td>
				<td><input name="id" type="hidden" value="${serverInfo.id}" /> <input name="bus_type" type="hidden" value="修改" /> <input
					name="serialno" type="text" placeholder="请输入序号" class="easyui-validatebox" data-options="required:true" value="${serverInfo.serialno}"></td>
				<td>新信息设备准用证编号</td>
			...
			<tr>
				...
				<td><input class="easyui-combotree" id="respondepart" name="respondepart" value="${serverInfo.respondepart}"
			...
			<tr>
				...
				<td><select class="js-example-basic-single js-states form-control" id="marcher" name="marcher" value="${serverInfo.marcher}"
