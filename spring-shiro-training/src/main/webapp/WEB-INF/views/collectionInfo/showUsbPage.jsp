...
<script type="text/javascript">
	...
	$(function() {
		...
		dataGrid = $('#dataGrid')
				.datagrid(
						{
							url : '${path }/combination/usbDataGrid',
							fit : true,
							striped : true,
							rownumbers : true,
							pagination : true,
							...
							columns : [ [
...

									{
										...
										formatter : function(value, row, index) {
											var str = '';
											<shiro:hasPermission name="/combination/usbEditPage">
											str += $
													.formatString(
											...
											<shiro:hasPermission name="/combination/usbDelete">
											str += '&nbsp;&nbsp;|&nbsp;&nbsp;';
											str += $
													.formatString(
	...
	function addFun() {
		parent.$.modalDialog({
			...
			width : 650,
			height : 250,
			href : '${path }/combination/addUsbPage',
			buttons : [ {
				text : '提交',
				handler : function() {
	...
	function deleteFun(id) {
		...
		parent.$.messager.confirm('询问', '您是否要删除该条记录？', function(b) {
			if (b) {
				progressLoad();
				$.post('${path }/combination/usbDelete', {
					id : id
				}, function(result) {
					if (result.success) {
	...
	function editFun(id) {
		...
		parent.$.modalDialog({
			...
			width : 650,
			height : 450,
			href : '${path }/combination/usbEditPage?id=' + id,
			buttons : [ {
				text : '提交',
				handler : function() {
					parent.$.modalDialog.openner_dataGrid = dataGrid;//因为添加成功之后，需要刷新这个dataGrid，所以先预定义好
...
<body class="easyui-layout" data-options="fit:true,border:false">
	...
	<div data-options="region:'center',border:true,title:'U盘列表管理'">
